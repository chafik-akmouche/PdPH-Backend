...
public class Solver /*extends JFrame*/ {
	...
	public Solver(int nb_semaines, double h_max,double hg_max,double OffD,double RepH,double Ratio_base[],double Ratio_dim_base[],int CDmax,
	...
	//System.out.println("Collez la route pour votre fichier de sortie (par exemple C:\\Users)");
	...
	for(int obj=1;obj<=3;obj++) {
	  ...
	  //Define new model
	        ...
	        try {
	    		...
	    		//cplex.setParam(IloCplex.Param.MIP.Limits.Solutions,1);//max number of solutions
	    		//cplex.setParam(IloCplex.Param.Emphasis.MIP, 4);//enfasis del MIP 0=balanced 1=feasibility 2=Optimality 3=best bound 4=hidden feas solutions
	    		//cplex.setParam(IloCplex.Param.MIP.Strategy.VariableSelect, 4);//seleccion de variable -1=minimum inf 0=auto 1=maximum infeasibility 2=pseudo costs  3=Strong branching 4=Branch based on pseudo reduced costs
		    	...
		    	if(Constrainte7) {
		    	  ...
		    	  //Constrainte8 Repos hebdomadaire de RepH une fois par semaine
					...
					for(int k=0;k<K;k++) {
		    	      for(int j=k*7;j<(k+1)*7;j++) {
						  for(int l=0;l<nb_contrats;l++) {
							  ...
							  cplex.addGe(Repos_Hebdo[j][l],cplex.sum(cplex.prod(RepH+12,X[P-1][j][l]),-12));  //Contrainte
					...
					for(int l=0;l<nb_contrats;l++) {
					   for(int k=0;k<K;k++) {
						   cplex.addGe(cplex.sum(X[P-1][k*7+5][l],X[P-2][k*7+5][l],X[P-1][k*7+6][l],X[P-2][k*7+6][l]),cplex.prod(2,Y[k][l]));
		    		...
		    		//Constrainte5-Constrainte6 le nombre d'heures par cycle
		    		 IloLinearNumExpr []Nb_Heures_Cycle = new IloLinearNumExpr[nb_contrats];
			    		...
			    		if(Constrainte5) {
			    		cplex.addLe(Nb_Heures_Cycle[l],37.5*K*Ratio[l]+(d_min-0.1));//Contrainte Constrainte5
			    		}
			    		if(Constrainte6) {
			    		cplex.addGe(Nb_Heures_Cycle[l],37.5*K*Ratio[l]-(d_min-0.1));//Contrainte Constrainte6
			    		}
			    		}
		    	    ...
		    	    if(Constrainte13) {
		    	    	//ancienne version
						   ...
						   for(int k=0;k<K;k++) {
							   for(int l=0;l<nb_contrats;l++) {
								   for(int i=0;i<Pj+Pn;i++) {
								  cplex.addLe(X[i][(k*7)+5][l],cplex.sum(X[i][(k*7)+6][l],cplex.prod(Math.abs(r[i][5]-r[i][6]),X[P-1][(k*7)+6][l]),cplex.prod(Math.abs(r[i][5]-r[i][6]),X[P-2][(k*7)+6][l])));//contrainte 1
								  cplex.addLe(X[i][(k*7)+6][l],cplex.sum(X[i][(k*7)+5][l],cplex.prod(Math.abs(r[i][5]-r[i][6]),X[P-1][(k*7)+5][l]),cplex.prod(Math.abs(r[i][5]-r[i][6]),X[P-2][(k*7)+5][l])));//contrainte 3
								  }
							}
					   }		 	    	
									...
									//Alternance de postes suite au changement d'un bloc postes (M,J,S)
					 				...
					 				IloLinearNumExpr [][][]Repos_ou_Jca= new IloLinearNumExpr[J][J+14][nb_contrats];
										...
										for(int j=0;j<J;j++) {
											...
											cplex.addLe(cplex.sum(X[i][j][l],X[i][k%J][l]),cplex.sum(k-j,cplex.negative(Repos_ou_Jca[j][k][l])));
									        }
										 }
								  ...
								  // Limiter les Jca couvrant le le poste ii
						    		...
						    		for(int i=0;i<Pj+Pn;i++) {
						    			for(int j=0;j<J;j++) {
						    				...
						    				cplex.addLe(Jca_type_i_couvert[i][j],Besoins_cycle[i][j]);//Contrainte limitant les Jca couvrant le type de poste ii								    			
						    			}
						    		}
						    	for(int j=0;j<J;j++) {
				...
				if(obj==3) {
				   ...
				   for(int l=1;l<nb_contrats;l++) {
					   for(int i=0;i<Pj+Pn;i++) {
						   cplex.addLe(Nb_postes_par_contrat[i][l],cplex.sum(NbpostesComp_par_contratUB[i][l],O3));//contrainte UB
						   cplex.addGe(Nb_postes_par_contrat[i][l],cplex.sum(NbpostesComp_par_contratLB[i][l],cplex.negative(O3)));//contrainte LB
					     }	   
				       }
	    	  ...
	    	  //solve model
	    		if(cplex.solve()) {
	    			...
	    			if (obj==1) {
	    				...
	    				System.out.println("Objectif 1 Resolu; Temps de calcul :"+(end.getTime()-start.getTime())/1000+" sec");	
	    			...
	    			if (obj==2) {
	    				...
	    				System.out.println("Objectif 2 Resolu; Temps de calcul :"+(end.getTime()-start.getTime())/1000+" sec");		    				
	    			...
	    			if (obj==3) {
	    				System.out.println("Objectif 3 Resolu; Temps de calcul :"+(end.getTime()-start.getTime())/1000+" sec");
	    			}		    				
...
//			            			System.exit(0); }        });	
		   		    	...
		   		    	String File_name="Cycle_Obj"+obj+".xlsx";
		   		 		try {
		   		 	    FileOutputStream fileout = new FileOutputStream(out+"\\"+File_name);//Nombre del archivo
	    			...
	    			Date end = new Date();
	    				...
	    				if((end.getTime()-start.getTime())/1000.>=time_limit[obj-1]) {
	    					System.out.println("Objectif "+obj+" non resolu; Temps de calcul :"+(end.getTime()-start.getTime())/1000+" sec");
	    				}
	    				
	    				if((end.getTime()-start.getTime())/1000.<time_limit[obj-1]) {
	    					System.out.println("Objectif "+obj+" infaisable; Temps de calcul :"+(end.getTime()-start.getTime())/1000+" sec");
	    				break;
	    				}
	    		}
	    		cplex.end();
